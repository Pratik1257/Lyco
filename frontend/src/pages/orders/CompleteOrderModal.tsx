...
export default function CompleteOrderModal({ isOpen, onClose, order }: CompleteOrderModalProps) {
  ...
  // Fetch full order data when modal opens to get existing files
  useState(() => {
    if (isOpen && order?.orderId) {
      ordersApi.getOrderById(order.orderId).then(fullOrder => {
        setExistingFiles(fullOrder.files || []);
        if (fullOrder.note) setNote(fullOrder.note);
        if (fullOrder.externalLink) setExternalLink(fullOrder.externalLink);
      }).catch(err => console.error('Failed to fetch existing files:', err));
    }
  });

  // Since order might change when modal re-opens
  const lastOrderId = useRef<number | null>(null);
  if (isOpen && order?.orderId && order.orderId !== lastOrderId.current) {
    lastOrderId.current = order.orderId;
    ordersApi.getOrderById(order.orderId).then(fullOrder => {
      setExistingFiles(fullOrder.files || []);
      setNote(fullOrder.note || '');
      setExternalLink(fullOrder.externalLink || '');
      setFilesToDelete([]);
    });
  }
